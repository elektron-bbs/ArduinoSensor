...
/*
  ...
  Protocoll AS
  -------------
  byte 0 		- Bit 0..6	sensor type	- Bit 7		trigger (0 auto, 1 manual)
  byte 1		- Bit 0..5	device ID		- Bit 6,7 battery status (00 - bad, 01 - change, 10 - ok, 11 - optimal)
  byte 2		- data lowbyte
...
asTransmitter::asTransmitter(byte typeIN, byte idIN, byte transmitterPinIN): SensorTransmitter(5, transmitterPinIN, 500, 19, 1, 2) {
  type = typeIN & 0x7f; //only bit 0..6
  id = idIN & 0x3f; //only bit 0..5
  setPauseTime(40);
}
